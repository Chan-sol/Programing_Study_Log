...
const clickBtn = (event) => {
    if(clickable) {
        ...
        $score.textContent = '';
        $score.append(`${userScore+cptScore}번째 판 ${message}!!`, document.createElement('br'), `${userScore}점(나) : ${cptScore}점(컴퓨터)`);
        if(userScore === 3 || cptScore === 3) {
            (userScore-cptScore)>0 ? $score.append(document.createElement('br'), '나 승리!') : $score.append(document.createElement('br'), '컴퓨터 승리!')
            return;
        }
        setTimeout(()=>{
            clickable = true;
