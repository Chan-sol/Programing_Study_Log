...
<?php
    while($data = mysqli_fetch_array($result)){
        ...
        $sql = "select count(*) as cnt from tb_reply where boardidx=$boardidx";
        $reply_result = mysqli_query($conn, $sql);
        $reply_data = mysqli_fetch_array($reply_result);
        $reply_count = $reply_data['cnt'];
?>
        <tr>
            <td><?=$boardidx?></td>
            <td><a href="./detail.php?boardidx=<?=$boardidx?>"><?=$boardtitle?></a> - [<?=$reply_count?>]</td>
            <td><?=$userid?></td>
            <td><?=$boardhit?></td>
            <td><?=$boardlike?></td>
            <td><?=$boardregdate?></td>
...
<?php
    //14            132         10
    $endPage = ceil($total/$pageCount); // 올림
    for($i=1;$i<=$endPage;$i++){
        if($page == $i){
            echo "[".$i."]";
        }
